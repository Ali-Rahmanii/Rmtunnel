...
### ترنسپورت KCP برای لینک‌های پرافت — حل شد با یکپارچه‌سازی paqet، نه بازنویسی
...
برای لینک‌های پرافت (که مسیر ایران↔خارج معمولاً همینه).

به‌جای بازنویسی یه موتور KCP/راو-پکت از صفر داخل این پروژه، تصمیم گرفته شد
که **[paqet](https://github.com/hanselime/paqet)** — یه پروژه‌ی جدا و
تست‌شده که دقیقاً همین کار رو می‌کنه (پکت‌های TCP خام + KCP + smux) —
مستقیم یکپارچه بشه: منو باینری واقعیش رو دانلود می‌کنه، کانفیگش رو با
ویزارد می‌سازه، و به‌عنوان سرویس مدیریتش می‌کنه. جزییات کامل توی
[README.md](../README.md)'s بخش «paqet» و [paqet.go](../paqet.go) هست.
بازنویسی همون منطق از صفر، تلاش تکراری بی‌فایده‌ای بود وقتی یه پیاده‌سازی
واقعی و تست‌شده از قبل وجود داره.
