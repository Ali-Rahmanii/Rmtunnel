...
func helpReverse() {
	...
	fmt.Println("  4. Install it as a service too. Check " + bold("Manage rmtunnel") + " on either box —")
	...
	fmt.Println(dim("Manage rmtunnel) catches the most common misconfigurations."))
	pressEnter()
}
...
func helpDirect() {
	...
	fmt.Println("  4. Install it as a service. Check " + bold("Manage rmtunnel") + " on either box.")
	fmt.Println()
...
func helpUDPCarrier() {
	...
	fmt.Println("     " + bold("Manage rmtunnel") + " → Status tunnels / Tunnel metrics for traffic.")
	fmt.Println()
	...
	fmt.Println()
	fmt.Println(yellow("⚠ the exposed ports carry UDP ONLY — no TCP listener exists on them at"))
	fmt.Println(yellow("  all. Testing with ssh/curl/a browser always shows \"connection refused\","))
	fmt.Println(yellow("  even when everything is working correctly, because those are TCP. Test"))
	fmt.Println(yellow("  with a UDP client instead — e.g. \"nc -u <iran-ip> <port>\" and type"))
	fmt.Println(yellow("  something, or just point the real UDP service (WireGuard, a game) at it."))
	pressEnter()
}
...
func helpPaqet() {
	...
	fmt.Println("  4. Both sides show up in " + bold("Manage rmtunnel") + " → Manage tunnel under \"paqet-server\"/")
...
func helpManage() {
	...
	fmt.Println("Main menu → " + bold("[3] Manage rmtunnel") + " has four screens:")
	fmt.Println()
	fmt.Println("  " + bold("Manage tunnel") + "  — the tunnel list and every per-tunnel action:")
	fmt.Println("      " + bold("Edit") + "     — change one thing (token, ports, disguises/KCP/QUIC, tier)")
	fmt.Println("                 and optionally restart right away. Anything needing the")
	fmt.Println("                 same change on the peer is flagged when you make it.")
	...
	fmt.Println("      " + bold("Restart ALL") + " and " + bold("File locations") + " act on every tunnel at once.")
	fmt.Println("  " + bold("Status tunnels") + " — a live dashboard (name/role/transport/state/ports)")
	fmt.Println("      that refreshes every few seconds — press Enter to return.")
	fmt.Println("  " + bold("Health check") + "   — root/systemd/BBR/qdisc, open file limit, token")
	fmt.Println("      strength, buffer values that exceed the OS socket-buffer ceiling,")
	fmt.Println("      reachability to each tunnel's peer, and port collisions between two")
	fmt.Println("      tunnels on this box.")
	fmt.Println("  " + bold("Link test") + "      — measures latency/jitter/packet loss to a dialing")
	fmt.Println("      tunnel's peer over its real disguise, then recommends (and can apply)")
	fmt.Println("      a transport change — always stating what the peer needs changed too.")
	pressEnter()
}
