...
func helpPaqet() {
	...
	fmt.Println("     preset and key, then the ports to forward — same format as rmtunnel's")
	fmt.Println("     own port question (1232, 1232:2323, or 1232=host:2323).")
	...
	fmt.Println(dim("paqet's own release binaries are statically linked)."))
	pressEnter()
}
